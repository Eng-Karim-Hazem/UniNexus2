...
class _QAScreenState extends State<QAScreen> {
  ...
  final List<Map<String, dynamic>> _questions = [
    {
      'id': 1, 'title': "IOT Arch.", 'subtitle': "GPIO pins on raspberry pi usage",
      'question': "How do I use the GPIO pins on raspberry pi and should i use an ADC for the project??",
      'answer': null, 'isExpanded': true,
    },
    {
      'id': 2, 'title': "Windows Programming", 'subtitle': "Button linking problem",
      'question': "My button click event is not firing in WPF. I checked the XAML binding. What could be wrong?",
      'answer': null, 'isExpanded': false,
    },
    {
      'id': 3, 'title': "CCNA R&S", 'subtitle': "Protocol mismatch issue",
  ...
  void initState() {
    super.initState();
    for (var q in _questions) { _controllers[q['id']] = TextEditingController(); }
  }
  ...
  void _onNavBarTapped(int index) async {
    ...
    else if (index == 1) { Navigator.push(context, MaterialPageRoute(builder: (context) => const StuSchedule())); }
    else if (index == 2) {}
    else if (index == 3) {
    ...
    return Scaffold(
      ...
      floatingActionButton: Container(
        ...
        child: FloatingActionButton(
          ...
          child: Container(width: double.infinity, height: double.infinity, decoration: BoxDecoration(shape: BoxShape.circle, gradient: _fabGradient), child: Padding(padding: const EdgeInsets.all(16.0), child: Image.asset('assets/images/qr_code.png', color: Colors.white))),
        ),
      ),
            ...
            children: [
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    GestureDetector(onTap: () => Navigator.of(context).popUntil((route) => route.isFirst), child: Image.asset('assets/images/menu.png', width: 28, color: _mainPurple)),
              ...
              Expanded(
                child: ListView.builder(
                  padding: const EdgeInsets.fromLTRB(20, 0, 20, 100), itemCount: _questions.length,
  ...
  Widget _buildQuestionCard(Map<String, dynamic> item) {
    int id = item['id']; bool hasAnswer = item['answer'] != null;

    return Container(
      decoration: BoxDecoration(color: Colors.white.withOpacity(0.75), borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.white.withOpacity(0.6), width: 1.5), boxShadow: [BoxShadow(color: const Color(0xFF237ABA).withOpacity(0.08), blurRadius: 15, offset: const Offset(0, 5))]),
      child: Theme(
        data: Theme.of(context).copyWith(dividerColor: Colors.transparent),
        child: ExpansionTile(
          initiallyExpanded: item['isExpanded'], tilePadding: const EdgeInsets.symmetric(horizontal: 20, vertical: 8), childrenPadding: const EdgeInsets.fromLTRB(20, 0, 20, 20),
          leading: Container(
            padding: const EdgeInsets.all(8), decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: const Color(0xFF5C5C80).withOpacity(0.3))),
          ...
          iconColor: const Color(0xFF1A1A1A), collapsedIconColor: const Color(0xFF1A1A1A),
          children: [
            const Divider(),
            Align(
            ...
            Container(
              ...
              child: TextField(
                controller: _controllers[id], style: const TextStyle(fontFamily: 'SpaceGrotesk'),
                decoration: InputDecoration(
                  hintText: "Submit an answer", hintStyle: TextStyle(fontFamily: 'SpaceGrotesk', color: Colors.grey.shade500), border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(horizontal: 20, vertical: 14),
                  suffixIcon: Padding(
                    padding: const EdgeInsets.all(4.0),
                    child: GestureDetector(
                      onTap: () => _submitAnswer(id, _controllers[id]!.text),
                      child: Container(decoration: BoxDecoration(color: const Color(0xFF5C7CFA), borderRadius: BorderRadius.circular(20)), child: const Icon(Icons.send, color: Colors.white, size: 18)),
                    ),
                  ),
                ),
              ),
            ...
            Align(alignment: Alignment.centerRight, child: Text("Who sent this?", style: TextStyle(fontFamily: 'SpaceGrotesk', fontSize: 12, color: Colors.grey.shade600, decoration: TextDecoration.underline))),
          ],
        ),
      ),
    );
